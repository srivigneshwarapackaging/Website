            ...
            {materials.map((material, index) => (
              <motion.figure key={material.name} {...reveal(index + 1)} className="group">
                <div className="relative aspect-[4/5] overflow-hidden bg-[#191817]">
                  <Image src={material.src} alt={`${material.name} — ${material.detail}`} fill sizes="(min-width: 1024px) 25vw, (min-width: 640px) 33vw, 100vw" className="object-cover transition-transform duration-700 group-hover:scale-[1.035] motion-reduce:transform-none" />
                </div>
        ...
        <div className="grid min-h-[680px] lg:grid-cols-2">
          <div className="flex items-end px-6 py-16 md:px-8 lg:px-16 lg:py-24">
            <div className="max-w-xl">
              ...
              <Link href="/products" className="mt-9 inline-flex min-h-11 items-center border-b border-[#d5a578] pb-2 text-sm font-medium text-[#f5f1e8] transition-colors hover:text-[#d5a578] focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[#d5a578]">
